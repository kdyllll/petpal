...
<div class="d-flex mb-3 editor">
	<div class="input-group mb-3">
		...
		<input type="text" class="form-control rounded" name="findComment" placeholder="칭찬과 격려의 댓글은 작성자에게 큰 힘이 됩니다:)" aria-label="Recipient's username" aria-describedby="button-addon2">
		<div class="input-group-append">
			<button class="btn btn-outline-secondary findWrite" type="button">등록</button>
		</div>
	</div>
</div>
<c:forEach items="${cList }" var="c">
	<c:if test="${c.COMMENTLEVEL eq 1 }">
		<div class="d-flex mb-2 ml-1 comment level1">
			...
			<c:if test="${c.STATUS eq 'E' }">
				<span class="ml-1"><c:out value="${c.FINDCOMMENT }" /></span>
			</c:if>
		...
		<div>
			...
			<c:if test="${c.STATUS eq 'E' }">
				<button class="reply click btn p-0 text-dark ml-2" style="font-weight: bold; font-size:14px;" value="${c.FINDCOMMENTNO }">답글 달기</button>
				<c:if test="${(loginMember.memberNo eq c.MEMBERNO) or (loginMember.memberNo eq '63') or (writeMember eq loginMember.memberNo) }">
					<button class="commentDelete click btn p-0 text-black-50" style="font-weight: bold; font-size:14px;" value="${c.FINDCOMMENTNO }">· 삭제</button>
				</c:if>
			</c:if>
		</div>
		</div>
	...
	<c:if test="${c.COMMENTLEVEL eq 2 }">
		<div class="d-flex mb-1 comment level2 ml-5 p-2 py-3 rounded mr-1 ${loginMember.memberNo eq c.MEMBERNO?'bg-point':'bg-light' }">
			...
			</a> <span class="ml-1"><c:out value="${c.FINDCOMMENT }" /></span>
		</div>
		<div>
			...
			<c:if test="${(loginMember.memberNo eq c.MEMBERNO) or (loginMember.memberNo eq '63') or (writeMember eq loginMember.memberNo) }">
			 <button class="comment2Delete click btn p-0 text-black-50" style="font-weight: bold; font-size:14px;" value="${c.FINDCOMMENTNO }">· 삭제</button>
			</c:if>
		</div>
		</div>
		</div>
