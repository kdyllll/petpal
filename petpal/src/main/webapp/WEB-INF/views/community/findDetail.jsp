...
<body class="bg-white">
	...
	<main role="main" style="min-height: 100vh;">
		<div class="container my-4 mt-lg-0" >
			...
			<div class="row mx-3 mb-5">
				...
				<div class="col-lg-9 pr-5 col-sm-12">
					...
					<strong><c:out value="${fDetail.TITLE }"/></strong></p>
						...
						<p >잃어버린 장소 : <c:out value="${fDetail.ADDRESS }" /></p>
						<c:if test ="${not empty findPics }">
							<div class="my-5" style="height:1px; width:100%; background: #dfe6e9;"></div>
							<c:forEach var="p" items="${findPics }">
								<div class="imgCon mt-5" >
									<div  class="position-relative rounded">
										<img src="${path }/resources/upload/find/${p.FILENAME}" class="col-12 mb-1 p-0 rounded" >									
									</div>
								</div>
				...
				<!-- 오른쪽 스티키-->
		        <div class="col-lg-3 d-none d-lg-block sticky-top py-0 " style="top:250px; height:200px; ">
			...
			<div class="row">댓글부분</div>
		</div>
		<!-- 작은사이즈 positon bottom에 fixed -->
		<div class="col-lg-3 d-block d-lg-none fixed-bottom" style="background-color: white; border-top:1px solid #dfe6e9;">
              <button type="button" class="btn btn-link mb-3">
                 <svg width="1em" height="1em" viewBox="0 0 16 16"
                  class="bi bi-heart" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                  <path fill-rule="evenodd" d="M8 2.748l-.717-.737C5.6.281 2.514.878 1.4 3.053c-.523 1.023-.641 2.5.314 4.385.92 1.815 2.834 3.989 6.286 6.357 3.452-2.368 5.365-4.542 6.286-6.357.955-1.886.838-3.362.314-4.385C13.486.878 10.4.28 8.717 2.01L8 2.748zM8 15C-7.333 4.868 3.279-3.04 7.824 1.143c.06.055.119.112.176.171a3.12 3.12 0 0 1 .176-.17C12.72-3.042 23.333 4.867 8 15z" />
              ...
              <div class="px-3">
                <div class="row d-flex justify-content-between mb-2"> 
                   ...
                   <button type="button" class="followBtn btn btn-sm bg-daily col-3 mr-2">팔로우</button>
                </div>
                <div class="d-flex justify-content-end align-items-center">
                  <span class="text-secondary" style="font-size:14px;"><c:out value="${fDetail.ENROLLDATE }"/></span>
                  <button type="button" class="btn btn-link text-secondary" style="font-size: 14px;" onclick="fn_claimModal('<c:out value="${fDetail.FINDNO}" />');">신고하기</button>
                </div>
              </div>
              <c:if test="${(loginMember.memberNo eq fDetail.MEMBERNO) or (loginMember.memberNo eq '63') }">
               <div class="d-flex justify-content-end mr-1">
