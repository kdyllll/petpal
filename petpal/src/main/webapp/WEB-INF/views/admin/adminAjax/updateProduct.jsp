					...
					<c:if test="${not empty pdt }">
						...
						<div class="form-group">
							<label for="exampleFormControlInput1">소카테고리</label> <input
								value="${pdt.SUBCATE }" type="text" class="form-control">
						</div>
						<div class="form-group">
							<label for="exampleFormControlInput1">상품이름</label> <input
								value="${pdt.PRODUCTNAME }" type="text" class="form-control">
						</div>

					</c:if>
					<c:if test="${not empty pimg }">
						<c:forEach var="pi" items="${pimg }">
							<input type="hidden" name="productImgNo" value="${pi.productImgNo }">
							<input type="hidden" name="imgNamez" value="${pi.imgName}" />
							<img src="${path }/resources/upload/product/detail/${pi.imgName}"
								title="사진변경" class="imgClick"
								style="cursor: pointer; width: 100px; height: 100px;" />
							<input type="file" name="imgNamez" class="d-none imgName" />
